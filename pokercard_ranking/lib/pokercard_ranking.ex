defmodule PokercardRanking do
  ...
  def process(options) do
    ...
    IO.puts(
      rankPlayers(rankHand(blackHandMap), blackHandMap, rankHand(whiteHandMap), whiteHandMap)
    )
  end
  ...
  def getCardID(card) do
    [value: _cardValue, card: cardID, type: _cardType, cardName: _cardName] = card
    cardID
  end

  def getCardValue(card) do
    [value: cardValue, card: _cardID, type: _cardType, cardName: _cardName] = card
    cardValue
  end
  ...
  def getCardName(card) do
    [value: _cardValue, card: _cardID, type: _cardType, cardName: cardName] = card
    cardName
  end

  def upCardTypeCounter(type, cardTypeCounterList) do
    [Hearts: heartsCount, Clubs: clubsCount, Diamonds: diamondsCount, Spades: spadesCount] =
      cardTypeCounterList

    cond do
      type == "Hearts" ->
        [Hearts: heartsCount + 1, Clubs: clubsCount, Diamonds: diamondsCount, Spades: spadesCount]

      type == "Clubs" ->
        [Hearts: heartsCount, Clubs: clubsCount + 1, Diamonds: diamondsCount, Spades: spadesCount]

      type == "Diamonds" ->
        [Hearts: heartsCount, Clubs: clubsCount, Diamonds: diamondsCount + 1, Spades: spadesCount]

      type == "Spades" ->
        [Hearts: heartsCount, Clubs: clubsCount, Diamonds: diamondsCount, Spades: spadesCount + 1]
  ...
  def highCardCalculation(p1CardMap, p2CardMap) do
    ...
    p1HighCardValue = getCardValue(p1HighCard)
    p1HighCardName = getCardName(p1HighCard)
    p2HighCardValue = getCardValue(p2HighCard)
    p2HighCardName = getCardName(p2HighCard)

    cond do
      p1HighCardValue > p2HighCardValue ->
        "Output: Black wins - high card: #{p1HighCardName}"

      p2HighCardValue > p1HighCardValue ->
        "Output: White wins - high card: #{p2HighCardName}"
  ...
  def processHighestCard(currentCard, previousCard) do
    ...
    previousCardValue = getCardValue(previousCard)
  ...
  def isStraight(cardMap) do
    ...
    [firstCard | [secondCard | [thirdCard | [fourthCard | [fifthCard | _possibleLeftOvers]]]]] =
      sortedKeyMap
  ...
  def areCardsFollowUps(cardOne, cardTwo, followUp) do
    cardOneValue = getCardValue(cardOne)
    cardTwoValue = getCardValue(cardTwo)
  ...
  def isThreeOfAKind(cardMap) do
    [firstCardCounter | [secondCardCounter | [thirdCardCounter | _leftovers]]] =
      countCards(cardMap)
  ...
  def countCards(cardMap) do
    ...
    firstCardValue = elem(firstCard, 1) |> getCardValue
    secondCardValue = elem(secondCard, 1) |> getCardValue
    thirdCardValue = elem(thirdCard, 1) |> getCardValue
    fourthCardValue = elem(fourthCard, 1) |> getCardValue
    fifthCardValue = elem(fifthCard, 1) |> getCardValue
    ...
    secondCardCounter =
      Enum.count(cardValuesList, fn x -> matchValue(x, secondCardValue) == 0 end)

    thirdCardCounter = Enum.count(cardValuesList, fn x -> matchValue(x, thirdCardValue) == 0 end)

    fourthCardCounter =
      Enum.count(cardValuesList, fn x -> matchValue(x, fourthCardValue) == 0 end)

    fifthCardCounter = Enum.count(cardValuesList, fn x -> matchValue(x, fifthCardValue) == 0 end)

    cardCountedList = [
      "#{firstCardValue}": firstCardCounter,
      "#{secondCardValue}": secondCardCounter,
      "#{thirdCardValue}": thirdCardCounter,
      "#{fourthCardValue}": fourthCardCounter,
      "#{fifthCardValue}": fifthCardCounter
    ]
  ...
  def listCards(hand) do
    ...
    [value: card1Value, name: card1Name] = Map.get(@allCardRankingMap, :"#{String.at(card1, 0)}")
    card1Type = Map.get(@allCardTypes, :"#{String.at(card1, 1)}")

    card2 = Enum.at(cardListStrings, 1)
    [value: card2Value, name: card2Name] = Map.get(@allCardRankingMap, :"#{String.at(card2, 0)}")
    card2Type = Map.get(@allCardTypes, :"#{String.at(card2, 1)}")

    card3 = Enum.at(cardListStrings, 2)
    [value: card3Value, name: card3Name] = Map.get(@allCardRankingMap, :"#{String.at(card3, 0)}")
    card3Type = Map.get(@allCardTypes, :"#{String.at(card3, 1)}")

    card4 = Enum.at(cardListStrings, 3)
    [value: card4Value, name: card4Name] = Map.get(@allCardRankingMap, :"#{String.at(card4, 0)}")
    card4Type = Map.get(@allCardTypes, :"#{String.at(card4, 1)}")

    card5 = Enum.at(cardListStrings, 4)
    [value: card5Value, name: card5Name] = Map.get(@allCardRankingMap, :"#{String.at(card5, 0)}")
  ...
  defp parse_args(args) do
    {options, _, _} = OptionParser.parse(args, switches: [arg: :string])
    options
  end
end
